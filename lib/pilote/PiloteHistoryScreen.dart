...
import './PiloteMainScreen.dart';
import 'package:shared_preferences/shared_preferences.dart';

class PiloteHistoryScreen extends StatefulWidget {
  const PiloteHistoryScreen({super.key});

  @override
  State<PiloteHistoryScreen> createState() => _PiloteHistoryScreenState();
}

class _PiloteHistoryScreenState extends State<PiloteHistoryScreen> {
  ...
  void _initWork() async {
    try {
      ...
      //bool e = (await preferences.getBool('login'))!;
      //print(e);
      //print('lllllllll');
      //preferences.getString('piloteId')
      piloteId = preferences.getString('piloteId')!;
      //var hpiloteId = await preferences.getString('piloteId');
      //print(hpiloteId);
      //print('lllllllll');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Historique: ${l} trajets',
    ...
    return GestureDetector(
      onTap: () {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => PiloteTrajetScreen(element.trajet.$id)));
      },
      child: Card(
            ...
            child: Column(
              ...
              children: [
                ...
                element.trajet.data['status'] == 'Termine'
                    ? Row(
                        children: [
                          Icon(Icons.send),
                          Text(
                              '${element.trajet.data['destination']['latitude']}, ${element.trajet.data['destination']['longitude']}')
                        ],
                      )
                    : SizedBox(
                        width: 150,
                ...
                Row(
                  ...
                  children: [
                    Text(
                      '${element.trajet.data['client']['nom']} ${element.trajet.data['client']['prenom']}',
